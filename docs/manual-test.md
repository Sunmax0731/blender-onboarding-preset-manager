...
- 作業ディレクトリ: `D:\AI\BlenderAddon\blender-onboarding-preset-manager`
...
2. `npm run validate` を実行し、`dist/blender-onboarding-preset-manager-representative-report.json` が生成されることを確認する。
3. レポート JSON を開き、happy-path、missing-required、warning、mixed-batch の summary が期待値通りであることを確認する。
4. Blender のスクリプト実行または add-on 読み込み前確認として src/host-adapter/blender_addon.py のメタ情報を確認する。
